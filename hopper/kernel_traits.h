...
struct Flash_fwd_kernel_traits : public Base {
    ...
    // hopper
    using elem_type_qk = elem_type;
    using ElementQK = elem_type_qk;
    // end
    ...
    // hopper
    using kSmemLayoutQ = decltype(tile_to_shape(
       GMMA::Layout_K_SW32_Atom<elem_type_qk>{}, make_shape(Int<kBlockM>{}, Int<kHeadDim>{})
    ));
    using kSmemLayoutK = decltype(tile_to_shape(
       GMMA::Layout_K_SW32_Atom<elem_type_qk>{}, make_shape(Int<kBlockN>{}, Int<kHeadDim>{})
    ));
    using kTiledMMA = decltype(make_tiled_mma(
       GMMA::ss_op_selector<elem_type_qk, elem_type_qk, ElementAccum, Shape<Int<kBlockM>, Int<kBlockN>, Int<16>>>(),
    ...
    using kSmemLayoutV = kSmemLayoutK;
    // Note this is the transpose in terms of the view, not in terms of memory.
    using kSmemLayoutVt =
        decltype(composition(kSmemLayoutV{},
                    make_ordered_layout(
                        make_shape(Int<kHeadDim>{}, Int<kBlockN>{}),
                        Step<_2, _1>{})));
    using kTiledMMA1 = decltype(cute::make_tiled_mma(
        cute::GMMA::rs_op_selector<Element, Element, ElementAccum, Shape<Int<kBlockM>, Int<kHeadDim>, Int<16>>,
    ...
    // end
    // hopper
    using kAsyncCpyPipe = cutlass::PipelineAsync<2>;
    static constexpr int kSmemKVSize = size(kSmemLayoutK{}) * 2 * sizeof(Element);
    static constexpr int kSmemSize = (Share_Q_K_smem ? std::max(kSmemQSize, kSmemKVSize) : kSmemQSize + kSmemKVSize) + sizeof(typename kAsyncCpyPipe::SharedStorage);
    // end
