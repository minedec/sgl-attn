...
inline int num_splits_heuristic_fa2(int batch_nheads_mblocks, int num_SMs, int num_n_blocks, int max_splits) {
...
std::tuple<at::Tensor, at::Tensor> set_params_splitkv(Flash_fwd_params &params, const int batch_size,
    ...
    if (p_dropout == 0.0f) {  // SplitKV is not implemented for dropout
        if (num_splits < 1) {
            ...
            params.num_splits = num_splits_heuristic_fa2(batch_size * num_heads * num_m_blocks, num_sm * 2, num_n_blocks, 128);
        }
...
void run_mha_fwd_sparse(Flash_fwd_params &params, cudaStream_t stream, bool force_split_kernel=false) {
    ...
    if(params.is_bf16) {
        ...
        BOOL_SWITCH(params.is_causal, Is_causal, [&] {
            run_mha_fwd_sparse_<cutlass::bfloat16_t, kHeadDim, Is_causal>(params, stream);
        });
        #endif
    } else {
        #ifndef FLASHATTENTION_DISABLE_FP16
        #ifndef FLASHATTENTION_DISABLE_HDIM128
        BOOL_SWITCH(params.is_causal, Is_causal, [&] {
            run_mha_fwd_sparse_<cutlass::half_t, kHeadDim, Is_causal>(params, stream);
        });
        #endif
        #endif
    }       
...
mha_fwd_sparse(at::Tensor &q,         // batch_size x seqlen_q x num_heads x head_size
    ...
    std::tie(softmax_lse_accum, out_accum) = set_params_splitkv(
        ...
        head_size_rounded, p_dropout, /*num_splits*/ 1, num_sm, opts);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("varlen_fwd_sparse", &mha_varlen_fwd_sparse, "Forward sparse pass (variable length)");
}
